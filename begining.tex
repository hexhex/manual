...
\begin{tabular}{ l r }
   \texttt{DLVHEX  2.4.0 [build Jun 23 2015   gcc 4.8.2]}& \\  
   &\\   
   \texttt{\{personOfInterest(john), friendOfDegree(john,0,john),}& \\
   \texttt{invite(john), friendOfDegree(john,1,mike),} & \\
   \texttt{friendOfDegree(john,1,david), friendOfDegree(john,1,charly),} & \\
   \texttt{invite(mike),invite(david),invite(charly)\}}
 \end{tabular}
...
\subsection{Summary}
To conclude our quickstart, let us summarize what did before. To solve our \emph{Social graph} problem we first provided set of facts representing instance. Second, we provided an encoding of the problem applying to any instance. With the encoding at hand, we could use \dlvhex{} to find an answer sets for the our problem. 

...
dlvhex.TUPLE as it is introduced in the previous section. Different input parameters are separated with  
``,'' sign   
...
direct friends of that person in the graph, which is loaded from the file in the $r_4$, and stores them to the linked 
list. All friends discovered, stored in the linked list, are sent to the output as it is specified in the $r_7$. 
...
In this example we 
have a predicate as input parameter. From the $r_2$ to 
...
function for the $\mathit{rq}$ external atom. We could here receive constants instead of predicate and only register function will be modified. \hex{} program will be affected a lot and may not be always possible to get desired functionality. This is explained in Section~\ref{example1}.  
\bigskip \\
...
r_1 \colon & import \ dlvhex \\
\\
r_2 \colon & def \ concat(tup) \colon\\
r_3 \colon & \ \ ret=\textsc{\char13}\textsc{\char13} \ \textsc{\char13}\textsc{\char13}  \\
r_4 \colon & \ \ for \ x \ in \ tup \colon \\
r_5 \colon & \ \ \ \ ret = ret + x.value() \\
r_6 \colon & \ \ dlvhex.output((ret, \ )) \\
\\
r_7 \colon & def \ register() \colon \\
r_8 \colon & \ \ dlvhex.addAtom(\textsc{\char13}\textsc{\char13}concat\textsc{\char13}\textsc{\char13}, (dlvhex.TUPLE, \ ), \ 1) \\
\end{align*}
\end{exmp}
...
$r_2$ to the $r_6$ in the example above. In the $r_3$ an 

...
\end{align*} 
 ...
 \\ \\F1 represents person for which we are looking the successors. \\F2 is variable which holds successor nodes of F1. \\P represent person of interest, in our case ``john".  
\\\\Deg and DegPlus are variables used to keep distance from head node \\ \\External atom \ext{friendsOf}{F1}{F2} has one input and one output parameter. For input F1, it finds all successor nodes of it and return them in F2. More details about plugin are discussed in Section 5. 
The fact
\begin{align*}
& \mathit{friendOfDegree(\text{P, Deg, F1})}
\end{align*}
is responsible for preparing F1 or node of interest for which we will find successor nodes. It will take always variable F1 from last iteration and that variable will be used as input to external source \ext{friendsOf}{F1}{F2}.
Result of current iteration is saved to 
\begin{align*}
& \mathit{friendOfDegree(\text{P, DegPlus, F2})}
\end{align*} 
where F2 contains all friends which should be processed in the following iterations if constraints allow it. There is one constraints which should be considered
\begin{align*}
& DegPlus < 2
\end{align*}
...
\begin{align*}
& \mathit{invite(\text{P})} \text{ v } \mathit{ninvite(\text{P}) \leftimpl friendOfDegree(\text{john,X,P}), \#int(\text{X}).}
\end{align*}     
We can limit number of invited persons by using \emph{Integrity constraint} of the following type.
\begin{align*}
& \leftimpl \mathit{not} \text{ 4 } = \mathit{\#count} \{ \text{P} : \mathit{invite(\text{P})} \}.
\end{align*} 
It gives 4 different sets of people which might me invited to the party. It is possible to replace last two statements with the following rule
\begin{align*}
& \text{ 4 } \leq \{ invite(\text{P}) : friendOfDegree(\text{john,X,P}) \} \leq \text{ 4 }. 
\end{align*}
It is doing exactly same thing as two statements above. However it is more powerful since it is possible to specify lower and upper bound on number of persons to invite and these numbers do not have to be same.  

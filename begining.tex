...
The $r_1$ specifies that the person who organizes the event and initializes the search. 

The rule $r_2$ specifies that initiating person has distance 0 from itself. The main computational part of the program is the rule $r_3$.  
...
\end{align*}
Binds to the variable $\mathit{F1}$ to a person for which we will find successor nodes. This value is sent as input to the external source $mathit{Deg}$ and $\mathit{DegPlus}$ are variables used to keep distance from head node \ext{friendsOf}{$F1$}{$F2$}, which returns all friends $F2$ of $F1$. For each such $F2$, we derive:
\begin{align*}

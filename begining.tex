...
some examples and the possibility to evaluate \hex{}-
programs directly in the browser, the online demo at 
...
We provide a script which should install \dlvhex{} 
...
additional options are available and explained in following 
sections.    
...
external source implementation (see 
Section~\ref{sec:externalInterfaces}). The output of the 
...
\\
r_3\colon& \\
r_4\colon& \mathit{comp\_edge}(X, Y) \leftimpl 
...
Classical negation, indicated by symbol \enquote{-}, is 
permitted in from of an atoms. The semantic relationship 
between $A$ and $-A$ is simply that they must not jointly 
hold.
...
for which $\mathit{day(t)}$ holds. Any variable occurring 
...
r_5\colon& \\
r_6 \colon & \mathit{in\_tree}(X,Y,C) \vee 
\mathit{out\_tree}(X,Y) \leftimpl \mathit{edge}(X,Y,C), 
\mathit{reached}(X). \\
r_7\colon& \leftimpl \mathit{root}(X), \mathit{in\_tree}
(\_,X,C).\\
r_8\colon& \leftimpl \mathit{in\_tree}(X,Y,C), 
\mathit{in\_tree}(Z,Y,C), X \neq Z. \\
r_9\colon&  \\
r_{10}\colon& \mathit{reached}(X) \leftimpl \mathit{root}
(X). \\
r_{11}\colon& \mathit{reached}(Y) \leftimpl 
\mathit{reached}(X), \mathit{in\_tree}(X,Y,C). \\
r_{12}\colon& \leftimpl \mathit{node}(X), \mathit{not} 
\mathit{reached}(X). \\
r_{13}\colon&   \\
...
Rule from the $r_6$ determines is an edge going from node 
$X$ (node $X$ is already reached) in the minimum spanning 
tree or out of it. Integrity constraint from $r_7$ ensures 
that there is no incoming edges to the root node. $r_8$ 
eliminates all answer sets where there are two outgoing 
edges going to same node $Y$. In the $r_{10}$ and $r_{11}$ 
we have rules that are deciding is node reached or not. 
$r_{11}$ removes all answer sets where there is any node 
...
r_6\colon&  \\ 
r_7\colon& \mathit{member}(X,p1) \vee \mathit{member}(X,p2) 
\leftimpl \mathit{employee}(X).\\
r_8\colon& : \sim \mathit{member}(X,P), \mathit{member}
(Y,P), X \neq Y, \mathit{not} \mathit{know(X,Y)}. \colon& 
[1:1] \\
r_{9}\colon& : \sim  \mathit{member}(X,P), \mathit{member}
...
the project 1 or project 2 what is given in $r_7$. Last 
...
which are encoded and solved in the \dlvhex{}. In the 
Section~\ref{example1} we have solved basic ``swimming 
problem". In the Section~\ref{traveling} we have presented 
``traveling salesperson" problem. As a last example we have 
Section~\ref{example3} which shows how we can use \dlvhex{} 
to plan route of one or multiple agents in the dynamic 
...
satisfy all constraints given from the user and place 
specifications which are obtained from the Internet using  
an external atom.
...
additional equipment she will need. Finally she has the 
...
r_6\colon& \mathit{need}(loc, C) \leftimpl 
...
($\mathit{inoutd}$ or $\mathit{loc}$) using predicate 
...
$\beta$ is required when swimming i a place in the 
...
outdoor location. By $r_5$ we ensure that one location is 
selected and integrity constraint from the $r_7$ ensures 
that only single location is selected. Integrity constraint 
in the $r_8$ removes all answer in which there is no chosen 
location to go. $r_6$ collects all requirements caused by 
...
single line. Since input parameter of the external atoms 
are different, external atom implementation changes. 
Implementation details of the plugin are discussed in 
Section~\ref{sec:externalInterfaces}.
...
explained in Section \ref{optimize}. In the classical 
solver we are able to load only small graphs because it is 
not feasible to load all graph at once. For this example we 
are using external atom which is loading graph from the 
external source edge by edge up to specified depth level 
what makes it able to solve problem with extremely large 
graphs.        
...
r_{11} \colon & lastCheck(B,C) \leftimpl budget(B), 
costCalculated(C), B \geq C. \\
r_{12} \colon & \leftimpl  budgetB(B), costCalculated(C), 
\mathit{not} \mathit{lastCheck(B,C).} \\
& \\
...
\end{align*}
\end{exmp}
In the $r_1$ we have a fact which specifies starting city. 
...
travelling all path should be at least same as budget 
available. An atom in $r_3$ keeps track of cities newly 
discovered, their distances from the root (starting city) 
and weight of the edge between newly discovered nodes and 
its predecessor. 
...
Rows $r_{13}$ and $r_{14}$, cyclically check whether nodes 
...
cycle given. Integrity constraint in $r_{15}$ eliminates 
all answer sets where all nodes in the subgraph are not 
reached.
...
answer set easier to read and understand.
...
\label{example3}
The last example is from the group of \emph{pathfinding } 
\emph{problems}. Pathfinding or pathing is the plotting of 
the shortest route between the two points. It searches a 
...
computer games \cite{multipleAgents}. We consider problem 
...
destination node for the agent $I$.  
...
valid or invalid. Agent can only know what are the edges 
available in the graph and make the guess in which 
direction he should go. Consider that an agent selects some 
edge but it become unreachable because of some reasons. 
These reasons may be: obstacle from the environment, the 
...
valid or invalid. In the case that any of the moves in the 
guessed path is invalid whole answer set is removed. If no 
...
$\mathit{agent(2)}$. Just to show different choices we are 

...
For every Python plugin two tasks are to be carried out:
\begin{compactitem}
\item
  Write a script that starts with \verb|import dlvhex|
  and implements a \verb|register| function
  that registers all external atoms.
\item
  For each external atom, write an evaluation function
...
\smallskip
The following Example~\ref{constantAsInput}
uses a constant (string) input parameter.
...
\smallskip
The following Example~\ref{predicateAsInput}
uses a predicate input parameter for the external atom \verb+&rq+
as required in Example~\ref{swimExample}.

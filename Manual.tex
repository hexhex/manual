...
\label{safetyCheck}
Errors in this section occurs if an input program is not safe.
\dlvhex{} imposes a safety condition on variables in rules. 
This guarantees that a rule is logically equivalent to the set of its Herbrand instances. Error appears if any of the variables used in the \hex{}-program does not hold for the rules given below. Examples and explanations in the following subsections are used from the \cite{brfwilvpg2009}. 

\subsubsection{Standard, Arithmetic and Comparative Predicates}
...
\end{align*}
\end{exmp}
To safely evaluate this kind of programs an upper integer limit N has to be specified either on the command-line (c.f Section~\ref{sec:commandline}) or in the program (with \#maxint=N.).


\subsubsection{Complex Terms}
...
\subsubsection{Cyclic rules}
By evaluating a program with external atoms it is possible to derive new numeric constants, different from those already occurring in the program. In case of cyclic rules, this could cause the non-termination of the evaluation and safety condition is violated. An error message is issued in this case. Example for this would be:
\begin{exmp} \textbf{Non bounded cyclic rule}
\begin{align*}
& d(0). \\
& d(Y) \leftimpl d(X), \ext{increment}{X}{Y}.
\end{align*}
\end{exmp}
Error generated is:
\begin{align*}
& \texttt{GeneralError: Syntax Error: Program is not liberally}\\
& \texttt{domain-expansion safe}
\end{align*}
Program given above is not safe because it can run infinitely many times, there is no any bound on an integer number which can be generated from the external atom. 

To solve this safety problem we have to introduce some small changes to the program. Adding another atom in the rule which is completely independent on the newly derived numeric constants makes the rule safe and program is \emph{strongly safe}.
\begin{exmp} \textbf{Bounded cyclic rule}
\begin{align*}
& d(0). \\
& d(Y) \leftimpl d(X), \ext{increment}{X}{Y}, lim(Y).
\end{align*}
\end{exmp}

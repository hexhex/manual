...
Consider that we data as specified in the Figure~\ref{fig:socialnetwork}.\\
\begin{figure}
\begin{center}
\begin{tikzpicture}
...
mechanism to easily add further external atoms, as introduce in Section~\ref{extatoms}, without 
...
which are implemented in the external sources (e.g. Python or C\# file). 

\emph{Exception} related with plugins occurs whenever we try to
load plugin from the nonexisting file, either from the python or C\# file.
\begin{align*}
...
right file which contains implementation for the external atoms used in \hex{}-program.

\emph{Error} related to plugins may occur if we do not provide an external atom specification and implementation 
in the external file but we use it in \hex{}-program. Error look as below:
\begin{align*}
& \texttt{GeneralError: Fatal: did not find plugin atom for predicate}\\ 
& \texttt{``ext\_atom''}
\end{align*}  
From the description above we can see that there is no plugin atom for predicate \texttt{ext\_atom}
...
Another error related with plugins may be if output arity of an external atom does not 
...
\end{align*}  
Output arity of an external atom in the \hex{}-program does not match its specification 
in plugin source file. Output arity which is registered and which should be used is given in the brackets. 

\subsection{Safety Checking}
\label{safetyCheck}
Errors in this section occurs if an input program is not safe. If any of the variables used in the \hex{}-program does not satisfy safety conditions listed below the program is not safe and error occurs. Examples and explanations in the following subsections are used from the \cite{brfwilvpg2009}. 

By evaluating a program with external atoms it is possible to derive new numeric constants, different from those already occurring in the program and this can generate a cycle. In case of cyclic rules, this could cause the non-termination of the evaluation and safety condition is violated. 

\subsubsection{Regular Safety}
\dlvhex{} imposes a safety condition on variables in rules. 
This guarantees that a rule is logically equivalent to the set of its Herbrand instances.
...
\end{align*}
\end{exmp}
To safely evaluate this kind of programs an upper integer limit N has to be specified either on the command-line (cf. Section~\ref{sec:commandline}) or in the program (with \#maxint=N.).
...
By evaluating a program with external atoms it is possible to derive new numeric constants, different from those already occurring in the program. Rules in which we have an external atom that derives new numeric constants and In case of cyclic rules, this could cause the non-termination of the evaluation and safety condition is violated.
...
restrictable. In this section we give an overview about the notion and refer to \cite{eite-etal-14a} for details.
\begin{exmp} \textbf{Consider the following program:}
\begin{align*}
& r_1 \colon p(a). \\
& r_2 \colon q(aa). \\
& r_3 \colon s(Y) \leftimpl p(X), \ext{concat}{X,a}{Y}. \\
& r_4 \colon p(X) \leftimpl s(X), q(X).\\
\end{align*}
\end{exmp}
It is not \emph{strongly safe} because Y in the cyclic external atom $\ext{concat}{X,a}{Y}$ in $r_3$ does not occur in an
ordinary body atom that does not depend on 
\\$\ext{concat}{X,a}{Y}$. When we run program above, with \texttt{--strongsafety} option (cf. Section~\ref{sec:commandline}) enabled, error that is generated looks as follow:
\begin{align*}
& \texttt{GeneralError: Syntax Error: Rule is not strongly safe! Variable}  \\
& \texttt{Y fails strong safety check in rule s(Y) :- p(X), \ext{concat}{X,a}{Y}.}
\end{align*} 
To resolve this error we should add an ordinary body atom that does not depend on output of the external atom.


However, program above is finitely restrictable as the
cycle is “broken” by $\mathit{q(X)}$ in $r_4$. Since the program is finitely restrictable it is also \emph{liberally-safe} and this concept is explained in the following section. 
...
has been introduced \cite{eite-etal-14a}, which incorporates both syntactic and semantic properties of a program. The
details of the notion are not necessary for this guide, except that all lde-safe programs have finite groundings
with the same answer sets; we give here a brief overview.
...
positive atoms as the original program) is finite. A program is lde-safe, if all its attributes are lde-safe.

Running the program above, with \texttt{--liberalsafety} option (cf. Section~\ref{sec:commandline}) enabled, executes successfully and outputs:
\begin{align*}
\{ & 

...
\begin{align*}
r_5 \prime  \colon \leftimpl \ not \ & 3 \leq \{ invite(P) \ \colon \ friendOfDegree(john,X,P)\} \leq 3.
\end{align*} 
...
otherwise. While the head atom $A_0$ of the fact 
...
deletes all color combinations which does not satisfy the 
...
for which $\mathit{day(t)}$ holds, i.e., whenever $\mathit{meet}$ holds, one of $\mathit{on(t)}$ for which $\mathit{day(t)}$ is true should hold. Consider that except $\mathit{on(X)}$ we have also an atom $\mathit{travel(X)}$. These two atoms are separated with ``;'' sign, the rule would look like:
...
The fact $r_1$ of the example above defines the root node 
...
In rule $r_{10}$ initial location for the path is set. Rule $r_{11}$ 
...
\emph{Exception} occurs whenever we try to
...
right file which contains implementation for the external atom(s) used in \hex{}-program.

\emph{Error} occurs if we do not provide an external atom specification and implementation 
in the source file but we use it in \hex{}-program. Error looks as below:
\begin{align*}
...
Another error may be if output arity of an external atom does not 
...
By evaluating a program with external atoms it is possible to derive new numeric constants, different from those already occurring in the program, and generate a cycle. In case of cyclic rules, this could cause the non-termination of the evaluation and safety condition is violated. 
...
\\$\ext{concat}{X,a}{Y}$. When we run program above, with \texttt{--strongsafety} option enabled (cf. Section~\ref{sec:commandline}), error that is generated looks as follow:
\begin{align*}
& \texttt{GeneralError: Syntax Error: Rule is not strongly safe! }  \\
& \texttt{Variable Var fails strong safety check in rule Rule.}
\end{align*} 
Along with error message, the affected \texttt{Rule} and a list of all unsafe variable occurrences \texttt{Var} are reported. 
...
Since the program from Example~\ref{strongSafetyExmp} is finitely restrictable, the cycle is “broken” by $\mathit{q(X)}$ in $r_4$, it is also \emph{liberally safe}. Program executes successfully with \texttt{--liberalsafety} option enabled (cf. Section~\ref{sec:commandline}) and outputs:
\begin{align*}
\{ & 
\texttt{p(a),q(aa),p(aa),s(aa),s(aaa)}
\}
\end{align*}
The details of the notion are not necessary for this guide, more informations about liberal safety is available at \cite{eite-etal-14a}.

...
\label{faceQuery}
\begin{align*}
\row{r_1}\colon & \mathit{personOfInterest}(\mathit{john}). \\
...
The symbolic set for the rule $r_8$ consists of 5 elements:\\ 
...
edges going to the same node $Y$. In $r_{8}$ and $r_{9}$ 
...
the project 1 or project 2 which is given in $r_6$. Rules $r_{7}$, $r_{8}$ and $r_{9}$ 
...
outdoor location. By $r_5$ we ensure that at least one location is selected. The integrity constraint $r_6$ removes all answer sets in which there is no selected location to go. Integrity constraint $r_7$ ensures that only a single location is selected since Alice can not go to different location at the same time. Rule $r_8$ collects all requirements caused by 
...
denoted as $C$. Rule $r_3$ defines the starting city for 
the path candidate. Rule $r_4$ is responsible for 
...
In rule $r_{10}$, the initial location for the path is set. Rule $r_{11}$ 
...
Since the program from Example~\ref{strongSafetyExmp} is finitely restrictable, the cycle is “broken” by $\mathit{q(X)}$ in $r_4$, it is also \emph{liberally safe}. The program executes successfully with \texttt{--liberalsafety} option enabled (cf. Section~\ref{sec:commandline}) and outputs:
\begin{align*}
\{ & 
